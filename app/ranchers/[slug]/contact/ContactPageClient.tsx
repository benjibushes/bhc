...
export default function ContactPageClient() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        const data = await res.json();
        // A rail redirect from the API is FOLLOWED, not rendered as an error —
        // same convention as the deposit page. A represented (broker-rail)
        // ranch refuses the message and points at its reserve section instead.
        if (data.redirectUrl) {
