...
//      Consumers has no free field for a stamp (new Airtable fields are not
//      an option), so the once-ever throttle is a Redis claimOnce (SET NX,
//      ~1yr TTL) taken BEFORE the send — a crashed run costs one email,
//      never a duplicate. Suppression (unsub/bounce/complaint) is checked
//      both in selection and again by the send wrapper.
...
async function realHandler(_request: Request): Promise<{
  ...
  for (const t of targets) {
    try {
      // Once-ever throttle, claim-BEFORE-send (see header). Keyed on the
      // consumer id so a re-captured email on the same row can't re-fire.
      ...
      if (result?.success) {
        ...
        // Suppressed at the send layer — the claim stays taken on purpose:
        // a suppressed buyer must not be retried daily forever.
        skipped++;
      }
