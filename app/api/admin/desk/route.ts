...
export async function GET(req: Request) {
  ...
  // F6 — Next Best Action. SUPPLY-AWARE (Wave 1B): the operational-rancher
  // coverage set gates hot/warm buyer suggestions so the engine never tells
  // the operator to dial a buyer nobody can serve. `ranchersActive` (Active +
  // signed) is a superset of the 6-gate operational set, so filtering it
  // through the canon costs no extra read.
  const coveredStates = new Set<string>();
  for (const r of ranchersActive as any[]) {
...
// buildDialCandidates moved to lib/dialCandidates (Wave 1B) — shared with the
// /admin/today cockpit so both surfaces fold sources into candidates the same
// way. rankDialQueue stays the single ranking authority either way.
