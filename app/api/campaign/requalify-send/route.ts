...
import { renderRequalifyEmail, validateRequalifyBatch, MAX_BATCH } from '@/lib/requalifyCampaign';
...
export async function POST(request: Request) {
  ...
  const { recipients, campaign, dryRun } = parsed;
  if (dryRun) {
    const first = recipients[0];
    const preview = renderRequalifyEmail(first.name, first.state);
  ...
  for (const r of recipients) {
    try {
      const rendered = renderRequalifyEmail(r.name, r.state);
