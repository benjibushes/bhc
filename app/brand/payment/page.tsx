  ...
  async function handlePayment() {
    ...
    try {
      ...
      if (data.url) {
        ...
        setError('Could not create checkout session. Please try again or contact support@buyhalfcow.com');
        setLoading(false);
      }
    } catch {
    ...
    return (
      <div className="max-w-lg w-full bg-white border border-[#A7A29A] p-8 md:p-12">
        ...
        <div className="bg-red-50 border border-red-200 p-4 text-sm text-red-800">
          Invalid payment link. Please contact support@buyhalfcow.com
        </div>
      </div>
    );
  }
