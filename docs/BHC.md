...
>
> **⭐ 2026-08-17 — MONEY MODEL 3: THE BROKER RAIL (the exception to buyer-pays-on-top).** Canonical source: `BUSINESS-MODEL.md` → "⭐ MONEY MODEL 3 — THE BROKER RAIL". Some ranchers are **represented by Ben with no platform footprint at all** — no Stripe Connect, no listing, no login. Ben sells their beef himself. On this rail the buyer's **deposit goes 100% to BHC's own Stripe account and IS the entire BHC fee** — there is no `application_fee`, no split, no invoice to the rancher. The rancher **nets price minus deposit**, collected directly from the buyer at pickup/delivery (cash/check/Venmo/Zelle — BHC never touches the balance). **COPY RULES:** buyer-facing copy NEVER frames the deposit as commission — it is "a deposit that reserves your share," full stop. Rancher-facing copy for broker deals says "you collect the balance at pickup"; never "keep 100%" (that's the Connect-rail line and it's false here). Any marketing generated for a broker-rail rancher that borrows Connect-rail money copy is WRONG — check which rail the rancher is on before writing money lines.
>
...
- **Generic / press:** "BuyHalfCow is the private network connecting families directly to verified ranchers. Real beef, straight from the ranch, no middleman." _(2026-06-15: dropped the "10% commission only on closed deals" tag — commission is now tier-based and, for tier_v2 ranchers, collected upfront on the deposit. Don't state a flat 10% in press.)_ _(2026-08-18: the trailing meat-pricing clause was struck. **BANNED in buyer copy:** any claim that BHC adds nothing to what the buyer pays — the markup family, "no cut", "you pay only the ranch's price". All of it is FALSE on the Connect rail, where the buyer's card is charged the platform fee ON TOP of the deposit (`lib/tiers.ts`: "the buyer pays our 10% on top"), baked into one unitemized line. "No middleman" is the true half — lean on that, plus "straight from the ranch". The struck claim stays sayable to RANCHERS, who really do keep 100% of their price. Pinned by `app/publicCopy.pins.test.ts`.)_
...
- **Rancher-facing (tier_v2):** "We send you qualified buyers from your own area until your capacity is sold out — you raise and fulfill, keep your brand and your customers. Free to start; pay as little as 0% commission." _(2026-07-08 NET-YOUR-NUMBER is live: on BHC checkout the application fee absorbs the card-processing estimate, so the rancher nets their number. Sayable: "card processing is on us — the net you see is the net you get." NEVER say "no Stripe fees" (Stripe is paid, by us), and never extend the claim to Operator DEPOSITS — 0% commission means nothing to absorb from; Operator products still qualify.)_ _(2026-07-08: "until you sell out" is now literally true — stale intros auto-expire and stop clogging capacity; lead flow only pauses on REAL live deals. Flagship pitch for serious share ranchers = the $500/mo Operator tier: "we close, you ship.")__(Legacy line, now tier-dependent: ~~"You close the deal. We take 10%."~~ — only true for un-migrated legacy ranchers. tier_v2 rates: Legacy Connect 10% · Pasture 7% · Ranch 3% · Operator 0%.)_
...
4. **Don't invent ad-hoc commission discounts.** The rate is **tier-based, never negotiated per-deal**: Legacy Connect 10% · Pasture 7% · Ranch 3% · Operator 0% (`lib/tiers.ts`; Champion Valley is live at 7%). The tier menu IS the negotiation — a rancher who wants a lower rate picks a paid tier. Never write copy promising a custom rate, and never present flat "10%" as THE rate. _(Restated 2026-08-17 — the old line "10% is the rate, we don't negotiate down" predates tier_v2.)_
5. **Don't use buyer names without consent.** First initial + last initial + state. That's it. _(⚠️ 2026-08-17 DISCREPANCY — live pages currently render buyers' full FIRST names (e.g. the latest-close strip and testimonial surfaces), while this rule says initials only. **PENDING BEN RULING** — do not "fix" either side until he calls it; don't cite this rule to strip first names from live pages, and don't cite live pages to loosen this rule in new copy.)_
