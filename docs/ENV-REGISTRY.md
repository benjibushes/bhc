...
> Auto-generated from a full code sweep 2026-07-14 · delta sweep 2026-08-13 @ 0e169e5 (added the ~17 vars that shipped since — incl. the APPLIED_CHASE / PIPELINE_SLA revenue rails awaiting Ben's flip). THE reference for "what env vars do I need."
> ⚠️ = load-bearing: silent absence breaks money or comms. The email-canary/watchdog rails monitor these.

**199 variables · 26 load-bearing** (+8 prod-set orphans documented in the last section — set in Vercel but read by no code). Owner legend: **ben-flips** = a business switch you toggle deliberately · **set-once** = key/secret, set correctly and forget (watchdog guards it) · **code-default** = safe fallback exists.
...
| `JWT_SECRET` | Signs every emailed magic link, member/rancher session, activate/decline token; missing → lib/secrets throws at import (most routes 500 loudly). (The old app/api/backfill/* hardcoded-fallback hole is GONE — those routes were deleted entirely; verified 2026-08-13) | fail-loud | set-once |
...
| `SENTRY_DSN` | Server/edge Sentry error capture (instrumentation.ts): "No DSN → no-op. The founder activates this by setting SENTRY_DSN." The Telegram operator-signal wire fires regardless — Sentry is the additive layer | fail-open | ben-flips |
...
| Var | Purpose | Fails | Owner |
|---|---|---|---|
| `ADAPTIVE_VARIANTS_ENABLED` | Tri-state A/B copy-variant switch for campaign sends (lib/campaignVariants.ts): only exact 'true' = live 50/50 arms, 'dry-run' = shadow (split computed + surfaced, variant A sent to everyone), anything else = off | fail-silent | ben-flips |
| `APPLIED_CHASE_ENABLED` | Tri-state applied-rancher chase rail (#608, lib/appliedChase.ts — "applications can never rot silently"): 'true' = live day-2/5 chase sends (cap 20/run, oldest first), 'dry-run' = selection + report only, unset/anything = OFF with a daily 'disabled' Cron Runs row. **Revenue rail shipped dark — awaiting Ben's flip** | fail-silent | ben-flips |
| `BUYER_AUTORESPOND` | Buyer reply auto-answer dial (lib/autoRespond.ts) — DEFAULT **'auto'** by code (Ben 2026-07-17 "flip everything"): machine sends template objection answers itself (voice-/STOP-guarded, off-template escalates). 'assisted' = stage + one-tap, 'off' = dark. Unset ≠ off — unset is LIVE | fail-open | ben-flips |
| ⚠️ `DEPOSIT_SMS_RESCUE_ENABLED` | **INVERTED default-ON kill-switch** (#502, deposit-request-nudge cron): the SMS rescue leg for unpaid deposits runs unless this is exactly 'false'. Same inverted family as MATCHING_ENABLED — deleting the var turns the rail ON, not off | fail-open | ben-flips |
| `PIPELINE_SLA_ENABLED` | Tri-state pipeline-SLA chase cron (#610, lib/pipelineSla.ts): 'true' = live stage-SLA breach alerts/chases, 'dry-run' = report only, unset/anything = OFF. **Revenue rail shipped dark — awaiting Ben's flip** | fail-silent | ben-flips |
| `RANCH_STAND_DIGEST_ENABLED` | 'true' → monthly ranch-stand digest emails send live (cron runs days 1-4 tiered window); anything else = dry (selection + Cron Runs note, zero sends) | fail-silent | ben-flips |
| ⚠️ `RESERVE_ABANDON_RECOVERY_ENABLED` | **INVERTED default-ON kill-switch** (deposit-request-nudge cron): the reserve-abandon chase ("the launch cohort's only chase") runs unless exactly 'false'. Deleting the var turns it ON | fail-open | ben-flips |
| `ROUTING_RADIUS_ENFORCE` | 'strict' → matching/suggest enforces the delivery-radius gate hard; unset = advisory | fail-open | ben-flips |
| `SETUP_LINK_UNDELIVERED_ENABLED` | 'true' → hourly setup-link-undelivered rescue cron goes live; unset = dark by design with a daily "disabled — set … once group C stamps Email Sends delivered/bounced" Cron Runs note. Blocked on Ben enabling Resend delivered/bounced webhook events (#457) | fail-silent | ben-flips |
...
| `CAMPAIGN_AUTOPILOT_ENABLED` | Tri-state campaign-autopilot cron (ADAPTIVE-MARKETING-DESIGN PR 2), FAIL-TO-OFF: unset/anything = OFF ('disabled' Cron Runs row daily), 'dry-run' = L0 shadow (full first-touch wave selection + logged plan, zero sends), 'true' = live waves via self-call to /api/campaign/requalify-send (rail stamp 'autopilot'). Ramp 30/day until 3 distinct live-send days, then 120/day (shared `DAILY_CAMPAIGN_BUDGET` — the endpoint enforces the same ceiling server-side). Auto-pauses fail-closed: complaints ≥3/7d, >5 hard bounces/24h, prior-run send-failure >10%, any telemetry read failure | fail-silent | ben-flips |
...
| `LOG_RETENTION_DAYS_OVERRIDE` | Capacity audit 2026-08-19. Per-table retention window override, `Table=days` comma list (e.g. `Email Sends=45,Cron Runs=14`). Exists so Ben can act on the documented recommendations in lib/logRetentionPlan.ts WITHOUT a deploy — most urgently Email Sends, whose 90d window converges to ~35,900 rows = 72% of the entire 50k base cap on its own. Refuses <7 days, non-numeric, and any table not already under retention (an env typo must never start deleting Consumers). Unset ⇒ compiled-in windows | fail-open | ben-flips |
| `LOG_RETENTION_PACING_MS` | Delay between 10-row DELETE batches; default 400ms = 2.5 req/s, HALF Airtable's 5 req/s per-base ceiling. Raise to throttle the drain during an incident without a deploy. LOWER values are clamped — a reckless setting can't push the cron past half the ceiling | fail-open | code-default |
| `BASE_CAPACITY_WATCH_ENABLED` | INVERTED, on by default: only the exact string 'false' disables the /api/cron/base-capacity alarm. The whole finding was 'nothing watches the 50k record cap', so this rail must not be dark-by-default | fail-open | ben-flips |
| `AIRTABLE_RECORD_CAP` | Record cap the capacity alarm measures against; default 50,000 (Airtable Team plan). Set this on a plan upgrade so the alarm stops crying wolf. Garbage/zero falls back to 50,000 — a zero cap would make every base look 100% full | fail-open | code-default |
| `AIRTABLE_RETRY_DEADLINE_MS` | Wall-clock budget for one Airtable operation's rate-limit retries; default 12,000ms. Replaces the old un-bounded 1→2→4→8→16→32s ladder that could hang a single request for 63 SECONDS. Beyond the deadline the ORIGINAL error is rethrown so callers' 5xx paths fire immediately. Garbage/non-positive falls back to the default | fail-open | code-default |
...
| `MARKETING_DOMAIN_WARMED` | Autopilot warmup release (PR 2): while `MARKETING_SEND_DOMAIN` is set, campaign-autopilot holds at ramp volume (30/day) until this is exactly 'true' (Ben stamps DKIM warmup complete, ~3-6 weeks after the DNS flip). Unset ⇒ still warming — fail-safe SMALL. No effect while marketing rides the apex | fail-silent | ben-flips |
...
| `MARKETING_SUNSET_ENABLED` | Tri-state P5′ sunset cron ('true'/'dry-run'/off; unset=dark, daily Cron Runs row written either way): 180d-unengaged re-permission ask (cap 25/run) → 30d-later suppression (cap 25/run) + 12mo never-engaged no-contact suppression (cap 50/run). Suppression = Consumers Unsubscribed flag + Notes marker | fail-silent | ben-flips |
...
| `SITE_URL` | Bare (non-NEXT_PUBLIC) override checked FIRST in 3 files (resend-inbound webhook, lib/productSettlement, lib/orderStatusLink) before falling to NEXT_PUBLIC_SITE_URL → default | fail-open | code-default |
| `NEXT_PHASE` | Next.js-provided build-phase marker ('phase-production-build'); read by /ranchers/[slug] to detect prerender context | fail-open | code-default |
| `NEXT_RUNTIME` | Next.js-provided runtime marker ('nodejs'/'edge'); gates Sentry init + node-only instrumentation | fail-open | code-default |
| `NO_ACTION_NUDGE_MAX_PER_RUN` | Qualified-no-action nudge batch cap, default 50 | fail-open | code-default |
| `READY_CHASE_MAX_PER_RUN` | READY-chase batch cap inside waiting-activation cron, default 25 | fail-open | code-default |
| `SETUP_LINK_UNDELIVERED_MIN_MINUTES` | Minutes before a sent-but-unstamped setup link counts undelivered, default 60 (floor 1) | fail-open | code-default |
...
| `NEXT_PUBLIC_SENTRY_DSN` | Client-side Sentry DSN (instrumentation-client.ts); unset → browser error capture off (no-op by design until the founder flips it) | fail-open | ben-flips |
