...
### Broker Fulfillment Steps + Broker Additional Costs (both multilineText) — added 2026-08-03
W: NO code writer — both are set by hand in Airtable, per represented ranch. Both OPTIONAL; blank is the common case and must render NOTHING (no empty heading, no fallback sentence — unlike `Broker Balance Note`, which always falls back to honest generic copy).
R: lib/brokerRail.ts `brokerFulfillmentSteps()` (splits on newlines, strips hand-typed list markers, caps at `BROKER_FULFILLMENT_STEPS_MAX`) and `brokerAdditionalCosts()` (trimmed, '' when unset) → projected by app/api/checkout/broker/route.ts GET onto the broker checkout page, and carried through `buildBrokerOrderFacts` (lib/brokerNotify.ts — the ONLY assembly point) onto both settlement emails.
Sem: fulfillment TRANSPARENCY, not money — neither field touches the deposit/balance arithmetic or eligibility. `Broker Additional Costs` is money the buyer pays a THIRD PARTY (a processor's cut-and-wrap bill, a hauler) on top of the share price; it is NOT the ranch balance (that is `Broker Balance Note`) and never anything owed to BHC. It is disclosed above the pay button and restated on the receipt, so the buyer can never first meet it as a surprise bill. The steps are echoed to the rancher labeled as what the BUYER was told, so both sides work from one script.
