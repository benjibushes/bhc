...
R: **lib/followUpQueue.selectDueFollowUps is the single definition of "due"** — consulted by app/api/admin/desk (the "⏰ Follow up today" section, ranked directly BELOW the callback bucket and ABOVE every cold bucket) AND app/api/cron/daily-digest (the morning Telegram block). Desk pre-filters in Airtable with `NOT({Next Follow Up At}='')` — "is it set", nothing more; the digest reuses the digest's existing unfiltered Consumers read and adds ZERO Airtable calls.
